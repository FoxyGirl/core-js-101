...
function isTriangle(a, b, c) {
  ...
  const arr = [a, b, c].sort((x1, x2) => x1 - x2);
...
function findFirstSingleChar(str) {
  const result = str.split('').reduce((acc, item) => {
    ...
    if (acc.match(regExp) !== null && acc.match(regExp).length > 1) {
...
function reverseInteger(num) {
  const strNum = String(num).split('').reverse().join('');
...
function getDigitalRoot(num) {
  ...
  const iterSum = String(num).split('').reduce((acc, item) => acc + Number(item), 0);

  return getDigitalRoot(iterSum);
}
